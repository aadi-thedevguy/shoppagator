...
import { forgotValidator, TForgotValidator } from '@/validators/account-credentials-validator'
...
const Page = () => {
  ...
  const onSubmit = async ({ email }: TForgotValidator) => {
    const { errors } = await forgotPassword({ email })
    if (errors) {
      return toast.error(errors)
    }
    return toast.success('Link sent successfully, Check your mail')
  }
