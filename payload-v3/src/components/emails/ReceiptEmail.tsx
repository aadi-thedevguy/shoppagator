...
export const ReceiptEmail = ({ email, date, orderId, products }: ReceiptEmailProps) => {
  ...
  return (
    <Html>
      <Head />
      <Preview>Your Shoppagator Receipt</Preview>
      ...
      <Body style={main}>
        <Container style={container}>
          <Section>
            <Column>
              <Img
                src="https://shoppagator.devvault.cloud/alligator-email-sent.png"
                width="100"
                height="100"
                alt="Shoppagator"
              />
          ...
          <Text style={footerLinksWrapper}>
            <Link href={`${process.env.NEXT_PUBLIC_SERVER_URL}/contact-us`}>Contact Us</Link> •{' '}
            <Link href={`${process.env.NEXT_PUBLIC_SERVER_URL}/tos`}>Terms of Service</Link> •{' '}
            <Link href={`${process.env.NEXT_PUBLIC_SERVER_URL}/privacy-policy`}>
              Privacy Policy{' '}
            </Link>
          </Text>
          <Text style={footerCopyright}>
            Copyright © {date.getFullYear()} Shoppagator Inc. <br />{' '}
