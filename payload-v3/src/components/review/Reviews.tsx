...
const Reviews = async ({ productId, averageRating, reviews, ratingCounts }: ReviewProps) => {
  ...
  return (
    <>
      ...
      >
        <div>
          <h3 className="text-xl font-semibold text-gray-900 sm:text-2xl">Customer Reviews</h3>
          ...
          <p className="text-muted-foreground text-sm my-2 px-4">
            {reviews.length > 0 ? reviews.length + ' people loved this product' : 'No Reviews Yet'}
          </p>
          {reviews.length > 0 && (
          ...
          <hr />
          <h4 className="text-lg mt-4 font-semibold text-gray-900 ">Customer Reviews</h4>
          ...
          <Button className="my-4">
            <Link
              href={
                user?.id ? `/write-review/${productId}` : `/sign-in?review&product=${productId}`
              }
            >
              Write a Review
            </Link>
